...
- `skills/phog/SKILL.md`: bundled skill for downstream agents using the CLI.

## Build, Test, and Development Commands

- `make` / `make build`: build `bin/phog`.
...
- Formatting: `make fmt` (`goimports` local prefix `github.com/kdubb1337/phog-cli` + `gofumpt`).
...
- Integration tests (local only):
  - `PHOG_IT_ACCOUNT=you@example.com go test -tags=integration ./internal/integration`
...
- Prefer OS keychain backends; use `PHOG_KEYRING_BACKEND=file` + `PHOG_KEYRING_PASSWORD` only for headless environments.
- `PHOG_ACCOUNT=<id>` selects the active account without `--account` on every command.
